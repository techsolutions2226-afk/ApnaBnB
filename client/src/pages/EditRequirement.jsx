/* ─── EditRequirement — Edit an existing property requirement ───
    Same form as PostRequirement, pre-filled with the requirement's saved data.
    Loads by :id, lets the user edit, then PUTs the update back to the API.
    ─────────────────────────────────────────────── */
...
import { CITIES, AREAS_BY_CITY, CITY_CENTERS, AREA_CENTERS } from "../config/locations";
...
// Normalize a stored propertyType onto the current subtype vocabulary.
...
// Split "10 Marla" / "1200 Sq. Ft." into a numeric value + unit.
const parseSize = (sizeStr) => {
  const raw = String(sizeStr || "").trim();
  if (!raw) return { size: "", sizeUnit: "Marla" };
  const m = raw.match(/^([\d.,]+)\s*(.*)$/);
  if (!m) return { size: "", sizeUnit: normalizeUnit(raw) };
  return { size: m[1], sizeUnit: normalizeUnit(m[2].trim() || "Marla") };
};
...
const EditRequirement = () => {
  ...
  /* ── Load requirement data on mount and seed the form ── */
  useEffect(() => {
    const load = async () => {
      if (!id) {
        setLoadError("No requirement ID provided");
        setIsLoading(false);
        return;
      }
      try {
        ...
        const propertyType = normalizeRestoredType(data.propertyType);
        const { size, sizeUnit } = parseSize(data.size);

        // Resolve city/area into dropdown + custom ("Other") fields.
        const rawCity = data.location?.city || "";
        const rawArea = data.location?.area || "";
        let city = "";
        let customCity = "";
        if (rawCity) {
        ...
        if (rawArea) {
          if (city === "Other") {
            ...
            if (knownAreas.includes(rawArea)) area = rawArea;
            else {
              area = "Other";
              customArea = rawArea;
            }
        ...
        const coords = data.location?.coordinates || null;
        ...
        setForm({
          ...
          area,
          customArea,
          category: data.category || CATEGORY_OF_SUBTYPE[propertyType] || "home",
          propertyType,
          budgetMin: data.budget?.min != null ? String(data.budget.min) : "",
          budgetMax: data.budget?.max != null ? String(data.budget.max) : "",
          size,
          sizeUnit,
        ...
        // Prefill the manual lat/lng inputs if a pin already exists.
        if (coords) {
        ...
        setLoadError(err.message || "Failed to load requirement");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // When the user types a free-text area ("Other"), geocode and recentre the
  // map there. Debounced; prefers Google Geocoder, falls back to Nominatim.
  useEffect(() => {
    let cancelled = false;
  ...
  /* ── Error State ── */
  if (loadError || !originalRequirement) {
    return (
      <div className="req-page">
        <div className="req-header" style={{ textAlign: "center", padding: "4rem 0" }}>
      ...
      <div className="req-header">
        ...
        <p className="req-subtitle">
          {isDealer
            ...
            : "Update what you're looking for — sellers and dealers will see your changes."}
        </p>
      </div>
